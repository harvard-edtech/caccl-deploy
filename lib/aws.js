...
const { ExistingSecretWontDelete } = require('./errors');
  ...
  if (assumedRoleCredentials === undefined) {
    try {
      ...
      const resp = await sts.assumeRole({
        ...
        RoleSessionName: 'deck-assume-role-session',
      }).promise();
...
const aws = {
  ...
  getRepoImageList: async (repo, all = false) => {
    ...
    if (!all) {
      ...
      const looksLikeSemver = new RegExp([
        '(?<Major>0|(?:[1-9]\\d*))',
        '(?:\\.(?<Minor>0|(?:[1-9]\\d*))',
        '(?:\\.(?<Patch>0|(?:[1-9]\\d*))))',
      ].join(''));
  ...
  getCfnStackOutputs: async (stackName) => {
    ...
    try {
      ...
      outputs = resp.Stacks[0].Outputs.reduce((obj, output) => {
  ...
  redeployEcsServcie: async (cluster, service, wait = true) => {
    const ecs = new AWS.ECS();
    try {
      ...
      let counter = 0;
      while (!allDone) {
        console.log(`still waiting (${counter})...`);
        counter += 1;
        await sleep(1000);
      }
