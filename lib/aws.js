...
const aws = {
  BASTION_LAUNCHER_FUNCTION_NAME:
    process.env.CACCL_DEPLOY_BASTION_LAUNCHER || 'PrivSecBastionLauncher',
  ...
  /**
   * Invoke the bastion launcher lambda to provision (or reuse) an ephemeral
   * bastion instance for an app.
   * @param {object} opts
   * @param {string} opts.appName
   * @param {number} [opts.ttlSeconds]
   * @returns {object} the launcher response, e.g.
   *   { instanceId, expiresAt, ttlSeconds, ... }
   */
  invokeBastionLauncher: async ({ appName, ttlSeconds }) => {
    const lambda = new AWS.Lambda();
    let resp;
    try {
      resp = await lambda
        .invoke({
          FunctionName: aws.BASTION_LAUNCHER_FUNCTION_NAME,
      ...
      if (err.code === 'ResourceNotFoundException') {
        throw new Error(
          `${aws.BASTION_LAUNCHER_FUNCTION_NAME} function not found in this account/region; has the PrivSecAgentInstallStack been deployed?`,
        );
      }
      if (err.code === 'AccessDeniedException') {
        throw new Error(
          `Not authorized to invoke ${aws.BASTION_LAUNCHER_FUNCTION_NAME}; ask for the TempBastionUserAccess IAM policy to be attached to your user/role`,
        );
      }
      throw err;
    }
    ...
    if (resp.FunctionError) {
      throw new Error(
        `bastion launcher failed: ${payload.errorType}: ${payload.errorMessage}`,
      );
    }
  ...
  /**
   * Poll until an instance's SSM agent reports itself online, meaning it can
   * accept session and command requests.
   * @param {string} instanceId
   * @param {object} [opts]
   * @param {number} [opts.timeoutSeconds]
   * @param {number} [opts.intervalSeconds]
   * @param {function} [opts.onTick] - called with elapsed seconds after each poll
   */
  waitForInstanceSsmOnline: async (instanceId, opts = {}) => {
    const { timeoutSeconds = 300, intervalSeconds = 10, onTick } = opts;
    ...
    while (elapsed < timeoutSeconds) {
      ...
      if (info !== undefined && info.PingStatus === 'Online') {
        return;
      }
    ...
    throw new Error(
      `Instance ${instanceId} SSM agent did not come online within ${timeoutSeconds}s`,
    );
  },
   ...
   */
  updateTaskDefAppImage: async (taskDefName, imageArn, containerDefName) => {
    const ecs = new AWS.ECS();
