...
const {
  ...
  promptAppImage,
  promptKeyValuePairs,
  promptLoadBalancerLogBucket: promptAlbLogBucket,
...
const funcs = {
  ...
  async generate(baseConfig = {}) {
    ...
    if (newConfig.albLogBucketName === undefined) {
      const defaultBucket = (
        await aws.getCfnStackExports(newConfig.infraStackName)
      ).albLogBucketName;
      newConfig.albLogBucketName = await promptAlbLogBucket(defaultBucket);
    }
